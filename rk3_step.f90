module rk3_step_func
    ...
    contains
        ...
        subroutine init_rk3_arrays(g,rk3_c)
            ...
            subroutine calc_a_b(g,ibm,rk3_c,n_loop)
            implicit none
            ...
            do k = 1,g%nz
                do j = 1,g%ny
                    do i = 1,g%nx
                        ...
                        ksi_u = ibm%coef_u_lap(i,j,k)*dt_loop / g%re
                        ksi_w = ibm%coef_w_lap(i,j,k)*dt_loop / g%re
            ...
            do k = 1,g%nz
                do j = 2, g%ny
                    do i = 1,g%nx
                        ...
                        ksi_v = ibm%coef_v_lap(i,j,k)*dt_loop / g%re
                        e_v = exp(ksi_v)-1.0d0
                        if (e_v >1e-10) then
        ...
        subroutine divU(f, g,rk3_c,n_loop)
...
#if defined(USE_IBM_G) || defined(USE_IBM) 
            call apply_ibm(f%un, ibm%coef_u, g,0,0,0)
            ...
            do k = 1, g%nz
                do j = 1, g%ny
                    do i = 1, g%nx
                        ...
                        f%ws(i,j,k) = (rk3_c%B_rk3_w(i,j,k) * f%wn(i,j,k) + g%dt*rk3_c%a(1)*f%mom_rhs_w0(i,j,k)&
                        -(dt_loop*dpz))/rk3_c%A_rk3_w(i,j,k)
#else
                        f%us(i,j,k) = (f%un(i,j,k) + g%dt*rk3_c%a(1)*f%mom_rhs_u0(i,j,k)-(dt_loop*dpx))
                        f%ws(i,j,k) = (f%wn(i,j,k) + g%dt*rk3_c%a(1)*f%mom_rhs_w0(i,j,k)-(dt_loop*dpz))
#endif 
                    end do 
                end do 
            end do 
...
#else
                        f%vs(i,j,k) = (f%vn(i,j,k) + g%dt*rk3_c%a(1)*f%mom_rhs_v0(i,j,k)-(dt_loop*dpy))
#endif
                    end do
                end do 
            end do 
            ...
            do k = 1, g%nz
                do j = 1, g%ny
                    do i = 1, g%nx
                        ...
                        g%dt*(rk3_c%a(2)*f%mom_rhs_w_int(i,j,k)+rk3_c%b(2)*f%mom_rhs_w0(i,j,k))-(dt_loop*dpz))&
                        /rk3_c%A_rk3_w(i,j,k)
#else
            ...
            do k = 1, g%nz
                do j = 1, g%ny
                    do i = 1, g%nx
                        ...
                        /rk3_c%A_rk3_u(i,j,k)
                        f%ws(i,j,k) = (rk3_c%B_rk3_w(i,j,k) * f%wn(i,j,k) +&
                        g%dt*(rk3_c%a(3)*f%mom_rhs_w_int(i,j,k)+rk3_c%b(3)*f%mom_rhs_w0(i,j,k))-(dt_loop*dpz))&
                        /rk3_c%A_rk3_w(i,j,k)
#else
                        f%us(i,j,k) = (f%un(i,j,k) +&
                        g%dt*(rk3_c%a(3)*f%mom_rhs_u_int(i,j,k)+rk3_c%b(3)*f%mom_rhs_u0(i,j,k))-(dt_loop*dpx))
